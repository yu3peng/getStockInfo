...
def getAllStockInfo(lst, stockURL, fpath):
    ...
    for stock in lst:
        ...
        try:
            if html=="":              #爬取失败，则继续爬取下一只股票
                continue
            p.apply_async(getStockInfo, args=(html, fpath))
            count = count + 1
...
def main():
    ...
    txt_file = now+r' GuChengStockInfo.txt'
    get_txt(txt_file)
    excel_file = now+r' GuChengStockInfoTest.xls'
    T_excel(txt_file,excel_file)
    time_cost = time.perf_counter() - start
    print("爬取成功，文件保存路径为:\n{}\n,共用时：{:.2f}s".format(excel_file,time_cost))
